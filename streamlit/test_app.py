...
def test_session_state():
    ...
    with col2:
        ...
        if st.button("Add Data Item"):
            st.session_state.test_data.append(f"Item {len(st.session_state.test_data) + 1}")
            st.rerun()
...
def main():
    """Main test function"""
    st.set_page_config(
        page_title="Smart Meeting System - Test",
        page_icon="🧪",
        layout="wide"
    )
    ...
    test_option = st.sidebar.selectbox(
        "Select Test",
        ["DataManager Test", "AuthManager Test", "Session State Test", "Integration Test"]
    )
