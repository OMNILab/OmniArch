...
def show_advanced_analytics():
    ...
    if analysis_type == "会议室使用效率分析":
        ...
        # 计算使用效率指标
        room_usage = merged_df.groupby('room_name').agg({
            'meeting_id': 'count',
        ...
        room_usage.columns = ['会议室', '会议次数', '总时长(分钟)', '总参与人数', '总成本']
        ...
        fig2 = px.scatter(room_usage, x='平均时长', y='平均参与人数', 
                         size='会议次数', hover_data=['会议室'],
                         title='会议室效率散点图')
        ...
        dept_analysis = merged_df.groupby('department').agg({
            'meeting_id': 'count',
        ...
        dept_analysis.columns = ['部门', '会议次数', '平均时长', '总时长', '平均参与人数', '总成本']
        ...
        fig = px.pie(dept_analysis, values='会议次数', names='部门', title='各部门会议数量分布')
        st.plotly_chart(fig, use_container_width=True)
        ...
        # 计算成本效益指标
        cost_analysis = merged_df.groupby('room_name').agg({
            'cost': 'sum',
            'duration_minutes': 'sum',
            'participants': 'sum'
        }).reset_index()
        
        cost_analysis['每分钟成本'] = cost_analysis['cost'] / cost_analysis['duration_minutes']
        cost_analysis['每人成本'] = cost_analysis['cost'] / cost_analysis['participants']
        ...
        fig = px.bar(cost_analysis, x='room_name', y='每分钟成本', title='会议室每分钟成本对比')
        st.plotly_chart(fig, use_container_width=True)
        ...
        daily_stats = meetings_df.groupby('date').agg({
            'meeting_id': 'count',
            'duration_minutes': 'sum',
            'participants': 'sum'
        }).reset_index()
...
if __name__ == "__main__":
    ...
    st.markdown("# 🤖 pandasAI 智能查询演示")
    st.markdown("本演示展示了如何在智慧会议系统中使用 pandasAI 进行智能数据查询和分析。")
