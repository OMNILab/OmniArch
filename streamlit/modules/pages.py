...
class Pages:
    ...
    def show_booking_page(self):
        ...
        with col1:
            self.ui.create_metric_card("今日会议", str(dashboard_data['meetings_today']))
        
        with col2:
            self.ui.create_metric_card("可用会议室", str(dashboard_data['available_rooms']))
        
        with col3:
            self.ui.create_metric_card("总会议数", str(dashboard_data['total_meetings']))
        
        with col4:
            self.ui.create_metric_card("平均时长", f"{dashboard_data['avg_meeting_duration']:.0f}分钟")
        ...
        with col1:
            ...
            today_meetings = meetings_df[pd.to_datetime(meetings_df['start_time']).dt.date == today]
            ...
            if len(today_meetings) > 0:
                ...
                for time_slot in time_slots:
                    ...
                    for _, room in rooms_df.head(3).iterrows():
                        ...
                        slot_time = datetime.strptime(f"{today} {time_slot}", "%Y-%m-%d %H:%M")
                        room_meetings = today_meetings[today_meetings['room_id'] == room['id']]
            ...
            for _, room in available_rooms.iterrows():
                ...
                with col1:
                    if st.button(f"预定 {room['name']}", key=f"book_{room['id']}"):
                        st.session_state.booking_form_data['selected_room'] = room['name']
                        st.success(f"已选择 {room['name']}")
                with col2:
                    if st.button(f"详情", key=f"details_{room['id']}"):
                        st.info(f"会议室详情: {room['name']} - 容量{room['capacity']}人 - 设备: {room['equipment']}")
        ...
        with st.form("booking_form"):
            ...
            with col1:
                ...
                meeting_date = st.date_input(
                    "会议日期", 
                    value=st.session_state.booking_form_data.get('date', datetime.now().date())
                )
                meeting_time = st.time_input(
                    "会议时间", 
                    value=st.session_state.booking_form_data.get('time', datetime.now().time())
                )
            ...
            with col2:
                ...
                room_selection = st.selectbox(
                    ...
                    index=rooms_df['name'].tolist().index(st.session_state.booking_form_data.get('selected_room', rooms_df['name'].iloc[0])) if st.session_state.booking_form_data.get('selected_room') in rooms_df['name'].tolist() else 0
                )
                meeting_type = st.selectbox(
                    "会议类型", 
                    ["项目讨论", "产品评审", "技术分享", "团队会议", "客户会议"],
                    index=["项目讨论", "产品评审", "技术分享", "团队会议", "客户会议"].index(st.session_state.booking_form_data.get('type', '项目讨论'))
                )
            ...
            with col2:
                if st.form_submit_button("确认预定", type="primary"):
                    if meeting_title and room_selection:
                        # Create new meeting
                        selected_room = rooms_df[rooms_df['name'] == room_selection].iloc[0]
                        ...
                        new_meeting = {
                            ...
                            'end_time': datetime.combine(meeting_date, meeting_time) + timedelta(hours=1),
                            'duration': 60,
                ...
                with st.expander(f"{meeting['title']} - {meeting['start_time']}"):
                    ...
                    with col3:
                        if st.button("取消预定", key=f"cancel_{meeting['id']}"):
                            self.data_manager.update_meeting_status(meeting['id'], '已取消')
                            st.success("预定已取消")
                            st.rerun()
        else:
            st.info("暂无预定记录")
    ...
    def show_minutes_page(self):
        ...
        meetings_df = self.data_manager.get_dataframe('meetings')
        meeting_options = [f"{row['title']} - {row['start_time']}" for _, row in meetings_df.iterrows()]
        
        if len(meeting_options) > 0:
            selected_meeting_option = st.selectbox("选择会议", meeting_options)
            selected_meeting_id = meetings_df.iloc[meeting_options.index(selected_meeting_option)]['id']
        else:
            st.warning("暂无会议记录")
            ...
            if uploaded_audio:
                ...
                if st.button("开始转写", type="primary"):
                    with st.spinner("正在转写音频..."):
                        ...
                        st.success("转写完成！")
                        st.session_state.minute_form_data['transcription'] = "这是转写后的会议内容示例..."
                    ...
                    with col2:
                        ...
                        with col1:
                            if st.button("确认", key=f"confirm_{minute['id']}"):
                                self.data_manager.update_minute_status(minute['id'], '已确认')
                                st.success("纪要已确认")
                                st.rerun()
                        with col2:
                            if st.button("发布", key=f"publish_{minute['id']}"):
                                self.data_manager.update_minute_status(minute['id'], '已发布')
                                st.success("纪要已发布")
                                st.rerun()
                        with col3:
        ...
        with st.form("task_form"):
            ...
            with col2:
                task_assignee = st.selectbox(
                    ...
                    index=users_df['name'].tolist().index(st.session_state.task_form_data.get('assignee', users_df['name'].iloc[0])) if st.session_state.task_form_data.get('assignee') in users_df['name'].tolist() else 0
                )
                task_priority = st.selectbox(
                    "优先级", 
                    ["高", "中", "低"],
                    index=["高", "中", "低"].index(st.session_state.task_form_data.get('priority', '中'))
                )
                task_deadline = st.date_input(
                    "截止日期", 
                    value=st.session_state.task_form_data.get('deadline', datetime.now().date() + timedelta(days=7))
                )
            
            if st.form_submit_button("创建任务", type="primary"):
                if task_title and task_description:
                    assignee_id = users_df[users_df['name'] == task_assignee].iloc[0]['id']
                    assignee_dept = users_df[users_df['name'] == task_assignee].iloc[0]['department']
            ...
            with cols[i]:
                ...
                for _, task in status_tasks.iterrows():
                    ...
                    with st.container():
                        ...
                        if next_status:
                            if st.button(f"→ {next_status}", key=f"update_{task['id']}_{status}"):
                                self.data_manager.update_task_status(task['id'], next_status)
    ...
    def show_dashboard_page(self):
        ...
        with col3:
            self.ui.create_metric_card("已完成任务", str(dashboard_data['completed_tasks']))
        ...
        if len(users_df) > 0 and len(tasks_df) > 0:
            dept_usage = tasks_df.groupby('department').agg({
                'id': 'count',
                'status': lambda x: (x == '完成').sum()
            }).reset_index()
    ...
    def show_settings_page(self):
        ...
        # Settings tabs
        tab1, tab2, tab3, tab4 = st.tabs(["用户管理", "组织架构管理", "系统配置", "数据管理"])
        ...
        with tab1:
            ...
            with col1:
                ...
                if len(users_df) > 0:
                    gb = GridOptionsBuilder.from_dataframe(users_df[['username', 'name', 'department', 'role', 'email']])
            ...
            with col2:
                ...
                if len(users_df) > 0:
                    ...
                    st.metric("管理员数", len(users_df[users_df['role'] == '系统管理员']))
                    st.metric("组织者数", len(users_df[users_df['role'] == '会议组织者']))
        ...
        with tab2:
            ...
            if len(users_df) > 0:
                # Organization data
                org_data = users_df.groupby('department').agg({
                    'id': 'count',
                    'name': 'first'
                }).reset_index()
        ...
        with tab3:
            ...
            with col2:
                ...
                if st.button("保存功能设置"):
                    self.auth_manager.update_user_preference('notifications', notifications)
        ...
        with tab4:
            ...
            with col2:
                ...
                if st.button("导出系统数据", type="primary"):
                    ...
                    json_data = json.dumps(all_data, default=str, ensure_ascii=False, indent=2)
                        ...
                        if "统计" in query or "概览" in query:
                            ...
                            if selected_source == '会议数据':
                                ...
                                if 'priority' in sample_data.columns:
                                    priority_counts = sample_data['priority'].value_counts()
                                    fig = px.pie(
                            ...
                            with col3:
                                if len(sample_data.select_dtypes(include=['number']).columns) > 0:
                                    st.metric("数值字段", len(sample_data.select_dtypes(include=['number']).columns))
                                else:
                                    st.metric("数值字段", 0)
                else:
                    st.warning("请输入查询内容")
